...
AdminWindow::AdminWindow(QWidget *parent, ClientController* controller) :
 QWidget(parent)
 {
...
this->Controller = controller;
 ...
 connect(shortcut_undo, SIGNAL(activated()), this, SLOT (slotHandleUndo()));
 connect(shortcut_redo, SIGNAL(activated()), this, SLOT(slotHandleRedo()));
 BookListUpdate();
}

void AdminWindow::slotHandleAdd(){
    std::string title;
    std::string author;
    Genre genre;
...
void AdminWindow::slotHandleRemove(){
    int index;
...
void AdminWindow::slotHandleUpdate(){
    std::string title;
    std::string author;
    Genre genre;
    std::string description;
    ...
    Book* book = new Book(title,author,genre,description,publicationYear,cover);
    this->Controller->updateBook(index,book);

    BookListUpdate();
}

void AdminWindow::slotHandleUndo()
{
    this->Controller->undo();
    BookListUpdate();
}

void AdminWindow::slotHandleRedo()
{
    this->Controller->redo();
    BookListUpdate();
}

void AdminWindow::BookListUpdate(){
    this->BookList->clear();
    int unsigned iterator = 0;
    std::string display;
    while (iterator < this->Controller->repo.vector.size()) {
        this->Controller->repo.vector.at(iterator)->toString(&display);
